                    ...
                    {payload.map((entry: any, index: number) => (
                        <div key={index} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "16px" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                                ...
                                <span style={{ fontSize: "0.875rem", color: "rgba(255,255,255,0.8)", transition: "color 0.1s ease" }}>{entry.name}</span>
                            </div>
...
// Premium Legend Formatter
const renderLegendText = (value: string, entry: any) => {
    return <span style={{ color: "rgba(255,255,255,0.7)", fontWeight: 500, marginRight: "16px", marginLeft: "4px" }}>{value}</span>;
};
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "32px", gap: "16px", flexWrap: "wrap" }}>
                ...
                {totalSent > 0 && (
                    <div style={{ 
                        ...
                        {totalResponses} response{totalResponses !== 1 ? 's' : ''} from {totalSent} message{totalSent !== 1 ? 's' : ''}
                    </div>
                )}
            </div>
                ...
                <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={data} margin={{ top: 10, right: 10, left: -28, bottom: 0 }}>
                        ...
                        <Legend 
                            ...
                            wrapperStyle={{ paddingTop: "0px", paddingBottom: "30px", fontSize: "0.75rem", textTransform: "uppercase", letterSpacing: "0.05em" }} 
                        />
                        <Area
                            type="monotone"
                            dataKey="sent"
